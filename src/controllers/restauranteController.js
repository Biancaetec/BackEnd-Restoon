...
export const createRestaurante = async (req, res) => {
    try {
        ...
        const result = await create(restauranteData);
        res.status(201).json({ message: "Restaurante criado com sucesso", id: result.lastInsertRowid });
    } catch (error) {
        if (error instanceof z.ZodError) {
            return res.status(400).json({
                message: "Erro de validação",
...
export const updateRestauranteStatus = async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ message: "Erro ao atualizar status de licenciamento" });
    }
};
