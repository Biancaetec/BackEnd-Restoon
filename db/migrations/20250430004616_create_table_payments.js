...
export async function up(knex) {
    return knex.schema.createTable('payments', function(table) {
      ...
      table.integer('user_control').comment('usuário que criou o registo');
      table.float('value').notNullable();
      table.text('receipt').notNullable();
      table.text('obs');
      table.timestamp('paymentdate').notNullable();
      table.boolean('verified').defaultTo(false).comment('se o pagamento foi verificado');
      table.text('photo').comment('link para a foto do comprovante de pagamento');
